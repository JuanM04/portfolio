...
const decodeHTMLEntities = (encoded: string) =>
  encoded.replace(/&#\d+;/g, entity => String.fromCharCode(parseInt(entity.slice(2, -1), 10)))
  ...
  parser: feed => {
    ...
    return items.filter(pubDateInRage).map((episode: any) => {
      const result: EpisodeType = {
        ...
        episode: [parseInt(episode["itunes:season"]), parseInt(episode["itunes:episode"])],
      }
...
const feeds: FeedItem[] = [
  {
    ...
    parser({ rss: { channel } }) {
      return channel.item.filter(pubDateInRage).map((episode: any): EpisodeType => {
