...
def generate_tool(name):
    image = Image.open(f"{og_path}/tool/background.png")
    draw = ImageDraw.Draw(image)
    font = ImageFont.truetype(f"{og_path}/tool/font.ttf", size=75)

    draw.text(
        (450, 310), name, fill="rgb(0, 0, 0)", font=font,
    )

    image.save(f"{final_path}/{name.lower()}.png")
...
def generate_doc(slug):
    ...
    image = Image.open(f"{og_path}/doc/background.png")

    draw = ImageDraw.Draw(image)
    font = ImageFont.truetype(f"{og_path}/doc/font.ttf", size=90)
...
def generate_all():
    if not os.path.exists(final_path):
        os.mkdir(final_path)
    for tool in ("BPM", "Docs", "LPNPCM", "To"):
        generate_tool(tool)
